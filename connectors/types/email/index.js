...
async function start(ctx) {
  ...
  async function processMessage(parsed) {
    ...
    let extra =
      ...
      `If this mail is to set up a meeting or calendar invite: do not create the Google event yet. Infer details, look up the address if they named a business, reply-all with the proposed title/when/who/where/description, and wait for James to confirm. If James does not name the event, guess a title from guests and context and show it in the repeat-back. If no end time, default to one hour. If the date is a US federal holiday, Good Friday, or Easter, say so on the thread. Do not tell a customer James is free or busy — busy conflicts go to James only (Discord). Remote only if James says remote (blank location). If he says house or office, use that address as location. Do not infer remote from home/office. Full flowchart: memory/ops/calendar-schedule.md. ` +
