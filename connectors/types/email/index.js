...
async function start(ctx) {
  ...
  async function processMessage(parsed) {
    ...
    let extra =
      `You are answering an EMAIL as ${fromName}. A letter-shaped reply is mailed on this thread (reply-all, quote, signature) unless you already asmltr send this turn, or you reply with only [[NO_REPLY]]. ` +
      `To send with flags: asmltr send email <addr> "body" --subject "${replySubject.replace(/"/g, '')}" [--cc "addr"] [--no-reply-all] [--new-thread] [--file path]. Prefer asmltr send when you need --new-thread, --no-reply-all, attachments, or a different To. Otherwise write the letter as your reply (greeting first). Do not put [[NO_REPLY]] on a letter — that token means stay silent and drops the body. ` +
      ...
      `If you already asmltr send this turn, or you should stay quiet, reply with exactly [[NO_REPLY]] and nothing else. Do not type a name or signature block — "${fromName}" and the rest of the signature are appended on send. NEVER sign as the operator/owner or impersonate a human. ` +
