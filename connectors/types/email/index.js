...
async function start(ctx) {
  ...
  async function processMessage(parsed) {
    ...
    let extra =
      ...
      `If this mail is to set up a meeting or calendar invite: do not create the Google event yet. Infer details, look up the address if they named a business, reply-all with the proposed title/when/who/where/description, and wait for James to confirm. If the date is a US federal holiday, Good Friday, or Easter, say so on the thread. Do not tell a customer James is free or busy — busy conflicts go to James only (Discord). Full flowchart: memory/ops/calendar-schedule.md. ` +
