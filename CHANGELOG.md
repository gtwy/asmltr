...
### Added
- **Discord guild ACP session.** Unmuted guild channels sleep by default and wake on a real `@mention` ping (not a bare name). While awake: queue inbound in receive order, no thought chips, medium effort, caret (`^`) scrollback/photo lookup, 10-minute idle 😴 sleep. `@mention stop` is a hard kill (starter or owner) while typing, and a gentle sleep (anyone) while idle. New channels default muted; existing mute map is unchanged. 8 Sep 2026.
- **Discord Ctrl+F.** `asmltr discord-search "<query>"` / MCP `asmltr_discord_search` uses official `GET /guilds/{guild.id}/messages/search` across every guild the bot is in (no history dump), then a capped `around` hop (≤25). Includes the assistant's own posts. On for everyone in public guild ACP; owner-private on DM / email / MCP. DMs use a capped around/before/latest window. Index-not-ready (202 / `retry_after`) is retried then surfaced. Food/cigar conversation digs go here, not Corona (Corona extras stay board-card only). 7 Sep 2026.
...
### Changed
- **Discord text ingress is ACP-only.** DM and guild text turns route to Grok. STANDARD (Claude SDK) stays in the tree for email, GitHub, schedules, and voice. `join-voice` is fully off until rebuilt. Public guild tool policy: search / guild-post / image on for everyone; shell / streams / write / uploads / arbitrary send off (card roles `trusted`/`email`/`mail` may email on-file peers). Operator in public gets the same tools plus `bypass_moderation`, with in-channel refuse → DM handoff. 8 Sep 2026.
