...
### Changed
- **Email owner Cc skipped when a covered staff addr is already on the letter.** Config `owner_cc_covered_by` (host: Joey). If Joey is To/Cc, keep him and do not auto-Cc the operator — including Joey + a customer. If Joey is not on the letter, usual outside-domain owner Cc. 9 Sep 2026.
- **Email calendar extra: guests from this inbound only.** Google attendees = other people on this message’s To/Cc, plus anyone named in this message. Do not mine the chain. First ivy@ letter matches how that inbound was addressed (no named-only adds). After book-it: originator only — guests already get Google’s invite. 9 Sep 2026.
- **Managed-install dashboard.** When `ASMLTR_UPDATE_MANAGED` or `~/.asmltr/managed` is set, Settings → Updates shows "managed by \<name\>" with no Update button, no auto-install toggle, and no "N commits behind" copy. The global banner is suppressed. `/v2/update/run` and auto-update already refused; status no longer git-fetches origin for telemetry. Matches [updater design](docs/UPDATER-DESIGN.md) issue #18.
- **Update status tracks this branch.** `getUpdateStatus` edge compares `origin/<current-or-configured branch>` via `update-ref`, same as the deterministic updater — not a hardcoded `origin/main`. A fork on `ivy` is not reported as N commits behind upstream main.
...
### Fixed
- **Email: closed-thread mute is harness, not prompt.** Owner saying a conversation is over writes `ivy-context/helpers/email-thread-mute.json`. Connector re-reads it on every inbound (no LLM turn from the customer) and every SMTP prepare (reply-gate and `asmltr send` both skip). Collector kill had no pid and `[[NO_REPLY]]` still mailed a customer 11–12 Sep. Owner/staff inbound on that thread still gets a turn. 12 Sep 2026.
- **Email: last-line `[[NO_REPLY]]` is silence again.** `5962778` stripped the sentinel and still returned the body so the reply-gate mailed it (customer stay-off, 11 Sep). Reverted that keep-letter path. Extra already says: write the letter with no token; the token means stay quiet / already `asmltr send`. Gate skips if the session text is or ends with the token. 11 Sep 2026.
- **Email: drop glued plan/thought above the letter.** Prompt-only `LETTER_ONLY_EXTRA` (5ec4406, 24 Aug) missed again (customer Outlook, 11 Sep). SMTP `buildMailContent` and the reply-gate now start at Hi/Dear Name, or drop leading Photo-is / I’ll-send scratch when a letter follows. No bare-name cut (`James,` / `Amazon,`). One-paragraph “I’ll send the invoice tomorrow.” stays. 11 Sep 2026.
- **Email: strip a trailing name-only sign-off** before the connector signature so a typed “Ivy” does not sit above Ivy Hedera. Extra: only To (or only recipient besides the sender) is for you even without your name. 11 Sep 2026.
- **Email extra: do not tag letters with `[[NO_REPLY]]`.** Extra used to say write the letter and end with the token; core last-line silence then dropped the body before the reply-gate. Extra: letter has no sentinel; token only after `asmltr send` or stay-quiet. The keep-letter core exception (`5962778`) is reverted — see above. 11 Sep 2026.
- **Grok one-shot `-s` needs a UUID.** Fresh `complete()` / title / status / self-assessment passed bare `-s`. Current grok CLI requires `--session-id <id>` and exits 2, so `/v2/self-assessment` 500'd every 5 min and session titles/status labels came back empty. Mint a UUID. 8 Sep 2026.
- **Discord search accent folding.** Official guild search treats `padron` and `padrón` as disjoint. `asmltr discord-search` / MCP `asmltr_discord_search` now expands each query (NFD-strip + common Spanish recombinations ó/ñ/é), searches each variant, and merges hits by message id. DMs fold both sides in the capped window. 7 Sep 2026.
- **Discord guild last-N on fresh resume.** Public guild turns re-inject last-N of that channel (`ASMLTR_DISCORD_GUILD_SCROLLBACK`, default 30) into the observe/catch-up preamble when the engine session is new. Reads per-channel `memory.json`; one `channel.messages.fetch({limit:N})` if empty/stale. Not search. DMs / PRIOR unchanged. 7 Sep 2026.
- **Email IMAP IDLE half-open flap.** After a bounce the watcher could NOOP while Gmail IDLE was half-open (15s timeout → close → 10s reconnect, ~73s cadence). Flap lines lived only in the manager's 200-line ring. Probe now breaks IDLE (DONE / `preCheck`) then NOOPs with a 35s timeout, skips while `fetchNew` is busy, sets ImapFlow `maxIdleTime` (10 min), backs off reconnect after 3 failures (cap 5 min), and appends probe/fail/reconnect JSONL to `~/.asmltr/email-imap-<instance>.jsonl`. `fails_hour` is on `/health`. UID cursor still survives reconnect. IDLE stays the new-mail path.
- **`asmltr bounce` on systemd:** the delayed restart starts in a new user scope (`systemd-run --user --scope`) so it is not in `asmltr-core.service`'s cgroup. `setsid` + `detached` only start a new session and stayed in the core cgroup; `KillMode=control-group` then killed the waiter when core stopped, so manager and collector never restarted. Restart order is collector → manager → core last. `--help` / `-h` / unknown flags no longer queue a bounce.
...
- **Email reply gate.** A letter-shaped session `reply` that never hit `/out` this turn is mailed through `buildOutPayload` (quote, reply-all, signature, owner Cc). Skip `[[NO_REPLY]]` / empty, `always_draft`, and ops matchers (never SMTP the vendor). `asmltr send` still marks the thread so the gate does not double. Extra points at `workflows/` / `medium/` (not silo `memory/ops/`). 9 Sep 2026.
