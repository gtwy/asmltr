...
### Fixed
- **Email: stay-off prose is not a letter.** Model wrote “James is talking to Markay, not to me — I’ll stay off this reply.” instead of `[[NO_REPLY]]`; the reply-gate SMTP’d it (22:56 EDT, after the letter-plan bounce). `looksLikeNonReply` now lives in `shared/silence.js`, matches stay-off / talking-to-X-not-me, and the email gate skips `stay-quiet`. A Hi/Dear greeting still mails. 11 Sep 2026.
...
- **Email letter + last-line `[[NO_REPLY]]`.** Extra told on-thread letters to end with `[[NO_REPLY]]` so the reply-gate could mail them; core treated last-line as universal silence and returned no reply action, so the gate never saw the body. Extra now: write the letter with no sentinel; `[[NO_REPLY]]` only after `asmltr send` or stay-quiet. Core on email strips a last-line sentinel and still returns the letter. Bare `[[NO_REPLY]]` still silences. 11 Sep 2026.
