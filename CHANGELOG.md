...
### Fixed
- **Email: closed-thread mute is harness, not prompt.** James saying a conversation is over writes `ivy-context/helpers/email-thread-mute.json`. Connector re-reads it on every inbound (no LLM turn from the customer) and every SMTP prepare (reply-gate and `asmltr send` both skip). Collector kill had no pid and `[[NO_REPLY]]` still mailed Markay 11–12 Sep. Owner/staff inbound on that thread still gets a turn. 12 Sep 2026.
- **Email: last-line `[[NO_REPLY]]` is silence again.** `5962778` stripped the sentinel and still returned the body so the reply-gate mailed it (Markay stay-off, 11 Sep). Reverted that keep-letter path. Extra already says: write the letter with no token; the token means stay quiet / already `asmltr send`. Gate skips if the session text is or ends with the token. 11 Sep 2026.
- **Email: drop glued plan/thought above the letter.** Prompt-only `LETTER_ONLY_EXTRA` (5ec4406, 24 Aug) missed again (Markay Outlook, 11 Sep). SMTP `buildMailContent` and the reply-gate now start at Hi/Dear Name, or drop leading Photo-is / I’ll-send scratch when a letter follows. No bare-name cut (`James,` / `Amazon,`). One-paragraph “I’ll send the invoice tomorrow.” stays. 11 Sep 2026.
