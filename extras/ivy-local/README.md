...
Contacts are **gworkspace** People API (`~/.asmltr/ivy-local/gworkspace/`), not Rolodex. Do not register a Rolodex MCP. Host JSON dumps under `~/.asmltr/rolodex-cache/` may sit on disk; Ivy does not read them.
...
`register.sh` creates `extras/ivy-local/.venv`, merges `~/.asmltr/mcp.json` (corona + onenote only), and runs `grok mcp add`. It does **not** add Rolodex or enable a rolodex sync timer.
