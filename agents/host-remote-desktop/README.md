...
|-------------------|------------------------------|------------------|-------------------------------------------------------------------|
| `broker_url`      | `ASMLTR_RD_BROKER`           | `-broker`        | **required** broker base URL, e.g. `https://asmltr.example.com` |
...
3. **Run it** (from a normal command prompt in the drop dir):
   ...
   ```
   host-remote-desktop.exe -broker https://asmltr.example.com -token <TOKEN> ^
