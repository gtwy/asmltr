...
test('parseContactsHasStdout known', () => {
  assert.equal(parseContactsHasStdout('{"ok":true,"known":true,"email":"a@b.com"}\n'), true);
});
...
test('emailsFromContactsDoc still parses an export shape', () => {
  const set = emailsFromContactsDoc({ results: [{ emails: ['A@B.com', 'x'] }] });
  assert.equal(set.has('a@b.com'), true);
