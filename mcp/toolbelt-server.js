...
const TOOLS = [
  ...
  { name: 'asmltr_who', description: 'Which sessions recently touched a specific file or directory — a targeted collision check for one path before you edit it.',
    ...
    argv: (a) => ['who', a.path] },
  { name: 'asmltr_send', deny: 'send', description: 'Deliver a message OUT through any connector (discord, telegram, email, …) to a target. In a Discord server, same-guild post: name looks up (confirm first), then id posts on-behalf-of. Email send is fire-and-forget (queued). On accept, reply on THIS channel immediately and stop. Do not wait on SMTP or another session.',
  ...
  { name: 'asmltr_guild_post', deny: 'guildPost', description: 'Alias of asmltr send for THIS Discord server only (trusted role / resolve allow, or Access 1–5 fallback). Target may be a name ("666 degree steak thread") or a snowflake. Names NEVER post — they return best-guess matches; ask the person to confirm, THEN call again with the id. Normal channels post in the channel (not a thread). Forum: thread = comment, forum channel = new post (pass title). Never this same channel. Works if the destination is muted for listening. Prefixed Posting on behalf of the asker. After a real post: Post complete. then [[NO_REPLY]].',
