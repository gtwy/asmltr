...
const TOOLS = [
  ...
  { name: 'asmltr_who', description: 'Which sessions recently touched a specific file or directory — a targeted collision check for one path before you edit it.',
    ...
    argv: (a) => ['who', a.path] },
  { name: 'asmltr_send', deny: 'send', description: 'Deliver a message OUT through any connector (discord, telegram, email, …) to a target. On success (including “already sent”), reply on THIS channel immediately and stop. Do not wait on another session. Do not send the same email again unless they said it never arrived (force=true).',
    inputSchema: { type: 'object', required: ['channel', 'target', 'text'],
      properties: { channel: { type: 'string', description: 'discord | telegram | email | …' }, target: { type: 'string', description: 'channel id / chat id / email address' }, text: { type: 'string' }, subject: { type: 'string', description: 'email subject (email only)' }, force: { type: 'boolean', description: 'resend the same email even if it just went out; only when they said it never arrived' } },
      additionalProperties: false },
    argv: (a) => ['send', a.channel, a.target, a.text, ...(a.subject ? ['--subject', a.subject] : []), ...(a.force ? ['--force'] : [])] },
