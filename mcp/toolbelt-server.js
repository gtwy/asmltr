...
const TOOLS = [
  ...
  { name: 'asmltr_send', deny: 'send', description: 'Deliver a message OUT through any connector (discord, telegram, email, …) to a target. Discord: a channel name looks up (does not post) until they confirm the id. Host overlay may add on-behalf-of and same-guild fence. Email send is fire-and-forget (queued). On accept, reply on THIS channel immediately and stop. Do not wait on SMTP or another session. Email: --new-thread is a blank new letter (no quote, no In-Reply-To). --no-reply-all only drops extra recipients and still quotes this thread. Mailbox Ccs the operator when any recipient is outside the operator\'s domain; staff on that domain do not auto-Cc the operator. Repeat file/files for multiple attachments on one letter (≤25MB total).',
    inputSchema: { type: 'object', required: ['channel', 'target', 'text'],
      properties: {
        ...
        file: { type: 'string', description: 'absolute path to attach' },
        files: { type: 'array', items: { type: 'string' }, description: 'more absolute paths on the same message; total ≤25MB' },
  ...
  { name: 'asmltr_discord_search', deny: 'discordSearch', description: 'Ctrl+F Discord via official guild search (GET /guilds/{id}/messages/search) — does not download channel history. Searches every guild this bot is in. Unaccented queries also search common accented variants (ó/ñ/é) and merge+dedupe by message id, so padron / pilon anejo hit accented posts. Optional channel_id prefers one channel as a search filter. Returns hits plus a capped around-hop (limit ≤25). Includes the assistant\'s own posts — do not skip bot/self. Owner-private turns only (owner DM, email, MCP). Denied on public guild bot turns. DMs: pass dm=true and channel_id; uses a capped around/before/latest window, never a full dump. For house food/cigar conversation, prefer a channel_id the operator or host INDEX named first; then other guilds; then general knowledge only if not already covered here. Do not use Corona for Discord conversation digs.',
    inputSchema: { type: 'object', required: ['query'],
      properties: {
        query: { type: 'string', description: 'Search text (Discord content filter, max 1024). Accents are folded: padron also finds padrón.' },
        channel_id: { type: 'string', description: 'Optional channel snowflake to prefer/filter (still official search, not a history dump)' },
        guild_id: { type: 'string', description: 'Optional single guild snowflake; default = every guild the bot is in' },
        around_limit: { type: 'integer', description: 'Context window around each hit (1–25)' },
        dm: { type: 'boolean', description: 'DM channel: capped around/before/latest page, not guild search' },
        around: { type: 'string', description: 'DM: message id to center the capped window' },
        before: { type: 'string', description: 'DM: message id to page backward (capped)' },
      }, additionalProperties: false },
    argv: (a) => ['discord-search', a.query,
      ...(a.channel_id ? ['--channel', a.channel_id] : []),
      ...(a.guild_id ? ['--guild', a.guild_id] : []),
      ...(a.around_limit != null ? ['--around-limit', String(a.around_limit)] : []),
      ...(a.dm ? ['--dm'] : []),
      ...(a.around ? ['--around', a.around] : []),
