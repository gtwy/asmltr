...
A **prompt** job at 08:00 weekdays: *"Write a warm ~25-word wake-up for [REDACTED] deliver it with
`asmltr notify` (read-aloud)."* → engine runs → asmltr notify → phone reads it over BT. Until notify
Part A ships, the prompt can call the existing `notify-owner` / android `/out` push instead.
