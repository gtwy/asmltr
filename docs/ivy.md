...
- Email: IMAP probe reconnects when the handle is dead (`!imap || !imap.usable`); do not return as a no-op. `connectImap` failures log, clear the handle, and retry in ~10s (`connecting` flag prevents overlap). Mid-fetch connection-class errors (`Connection not available`, not connected, socket, closed, timeout) call `close()` so the close handler reconnects. Always clear `busy` in `finally`.
