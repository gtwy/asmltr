...
- Do not drop whitespace-only grok tokens. `joinText` concatenates; if either side already has adjoining whitespace, keep it. Do not invent a space after `.!?`. If grok did not emit a space, do not invent one (`time.`+`The` stays `time.The`; `time.`+` `+`The` is `time. The`). Inventing a space mangled OAuth URLs (`accounts. google.com`, `127. 0. 0. 1`, `auth? response_type`).
