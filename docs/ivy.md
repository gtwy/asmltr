...
- Public Discord never posts raw Grok thoughts. `stream_steps` posts a sanitized `-# 💭 …` chip for a closed thought block (secrets redacted; drop the whole bubble on `CURRENT SPEAKER` / `identity.md` / `CLAUDE.md` / emails / `/home/` paths; clamp ~280). If the first bubbles are dropped, `-# Working` once. If 45s pass with no new chip, `-# Still working`. Human tool-start chips stay; `stream_tools` defaults false. Sanitize/drop is Discord display only — core `onThinking`, Live/web frames, and collector records stay full-fidelity. Email/MCP do not get thoughts.
