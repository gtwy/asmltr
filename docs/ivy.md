...
- Grok live steps: a tool call emits the closed narration as a segment (Discord `-#`, like Eve) and collector records thinking + tools so dashboard Live is not inbound→outbound with a blank middle. Incremental deltas never fired `onSegment` on their own. Last-block-wins still drops duplicate sentences. ivy-discord `stream_tools` on. Needs core + discord instance restart.
- Grok thought summaries (the grok.com chips) close as a block on tool/answer, then Discord posts `-# 💭 …` and dashboard Live/web chat show 💭. Not emailed. Needs core + discord restart.
