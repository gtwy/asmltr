...
- max-turns by effort: medium 20, high 40, xhigh 60 (cap 100). Inbound email xhigh is 100 turns and 60 minutes. Discord/generic timeouts: medium 10 min, high 10 min, xhigh 15 min (not 30 via scale). Timeout scales from the 10-minute baseline (watchdog cap 60m so email can use the hour). Not infinite. Take `ASMLTR_GROK_TIMEOUT_MS=600000` as that baseline. Idle stays 30 minutes.
