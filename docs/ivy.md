...
- max-turns by effort: medium 20, high 40, xhigh 60 (cap 100). Email stays 60 turns (not a 25-turn cap). Timeout scales from the 10-minute baseline so 40/60 can finish (cap 30m). Email xhigh is 25 minutes (generic xhigh stays 30). Not infinite. Take `ASMLTR_GROK_TIMEOUT_MS=600000` as that baseline.
