...
- Keep period-space when joining grok narration blocks. Space-only tokens were dropped as empty; later sentences arrived without a leading space (`time.The`). Only insert that space when the next token starts with an uppercase letter (sentence boundary). Do not space after `.`/`?` into a lowercase letter or digit — that mangled Discord OAuth URLs (`accounts. google.com`, `127. 0. 0. 1`, `auth? response_type`).
